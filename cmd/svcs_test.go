...
func TestExtractDatacenterFromEndpoint_OpenshiftPattern(t *testing.T) {
	dc := extractDatacenterFromEndpoint("https://api.app1.lab1.ocp.example.com:6443/healthz")
	if dc != "lab1" {
		t.Errorf("Expected 'lab1', got '%s'", dc)
	}

	dc = extractDatacenterFromEndpoint("https://api.app1.rdu1.ocp.example.com:6443/healthz")
	if dc != "rdu1" {
...
func TestExtractDatacenterFromEndpoint_ElasticPattern(t *testing.T) {
	dc := extractDatacenterFromEndpoint("https://es-master-01d.lab1.corpnet.us:9200/")
	if dc != "lab1" {
		t.Errorf("Expected 'lab1', got '%s'", dc)
	}

	dc = extractDatacenterFromEndpoint("https://es-master-01e.rdu1.corpnet.us:9200/")
	if dc != "rdu1" {
		t.Errorf("Expected 'rdu1', got '%s'", dc)
	}
}
...
func TestExtractDatacenterFromEndpoint_IdmPattern(t *testing.T) {
	dc := extractDatacenterFromEndpoint("https://idm-01a.lab1.examplecorp.local:443/")
	if dc != "lab1" {
		t.Errorf("Expected 'lab1', got '%s'", dc)
	}

	dc = extractDatacenterFromEndpoint("https://idm-01b.bru1.corpnet.us:443/")
	if dc != "bru1" {
