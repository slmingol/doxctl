...
# DNS Configuration
domNameChk: "example.local"     # Domain name to check
domSearchChk: "[0-1].*example"  # Search path regex pattern
domAddrChk: "[0-1].*10.5"        # Address regex pattern
domainName: "examplecorp.local" # Primary domain
...
wellKnownSvcs:              # Services and their servers
  - svc: openshift
    svrs:
      - ocp-master-01{a,b,c}.{lab1,rdu1}.examplecorp.local
