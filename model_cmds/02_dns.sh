...
dnsResolverChk() {
    ...
    (
        echo "$vpn_resolvers" | grep -q 'DomainName.*example.local' && echo "DomainName set" || echo "DomainName unset"
        echo "$vpn_resolvers" | grep -A1 'SearchDomains' | grep -qE '[0-1].*example' && echo "SearchDomains set" || echo "SearchDomains unset"
...
dnsResolverDigChk() {
    ...
    for site in $sites; do
        for resolver in $resolver_ips; do
            ...
            hitCnt1=$(dig @${resolver} idm-01a.${site}.examplecorp.local +short | head -1 | wc -l | tr -d ' ')
            hitCnt2=$(dig @${resolver} idm-01b.${site}.examplecorp.local +short | head -1 | wc -l | tr -d ' ')
...
### https://example.slack.com/archives/C01A01GQ1JM/p1620317818186000
